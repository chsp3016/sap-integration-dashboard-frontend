...
function dashboardReducer(state, action) {
  switch (action.type) {
    ...
    case DASHBOARD_ACTIONS.ADD_CUSTOM_TILE:
      ...
      localStorage.setItem('customTiles', JSON.stringify(newCustomTiles));
      return {
        ...state,
    ...
    case DASHBOARD_ACTIONS.REMOVE_CUSTOM_TILE:
      ...
      localStorage.setItem('customTiles', JSON.stringify(filteredTiles));
      return {
        ...state,
...
export function DashboardProvider({ children }) {
  ...
  useEffect(() => {
    const savedTiles = localStorage.getItem('customTiles');
    if (savedTiles) {
      try {
